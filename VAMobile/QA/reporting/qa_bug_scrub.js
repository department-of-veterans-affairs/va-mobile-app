...
const findProject = (config, keywords) => {
  ...
  return projects.find(p => keywords.some(kw => p.title.toLowerCase().includes(kw)))
      ?? null;
};
...
const fetchProjectItems = (config, projectId) => {
  ...
  while (true) {
    ...
    if (!result.ok) { log(`  → Project items error: ${result.error}`); return { statuses: {}, items: {} }; }
    ...
    for (const item of conn.nodes) {
      ...
      const iterField = item.fieldValues.nodes.find(
        fv => fv.startDate && fv.title
      );
...
const filterApprovedPRs = (prs, sinceStr) => {
  return prs.filter(pr =>
    pr.reviews.nodes.some(r =>
      r.state === 'APPROVED' &&
      QA_TEAM.has(r.author?.login) &&
      r.submittedAt >= sinceStr
    )
  );
};
...
const renderReport = (model) => {
  const { config, approved, sentBack, onDeck, forecast, totalTaggedIssues,
          qaApprovals, qaSendBacks, qaDeterminations, hasProjectData, hasSprintData } = model;
...
const main = () => {
  ...
  const projectData = config.includeProject
    ? fetchProjectData(config)
    : { prStatuses: {}, issueData: {} };
