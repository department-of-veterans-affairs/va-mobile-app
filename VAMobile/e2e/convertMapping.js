// Simple Node.js script to convert TypeScript mapping to JSON
const fs = require('fs')
const path = require('path')

// Read the TypeScript file as text and extract the data
const tsFilePath = path.join(__dirname, 'detoxMapping.ts')
const tsContent = fs.readFileSync(tsFilePath, 'utf8')

// Extract the fullTestRun array
const fullTestRunMatch = tsContent.match(/const fullTestRun = \[([\s\S]*?)\] as const/)
if (!fullTestRunMatch) {
  console.error('Could not find fullTestRun array')
  process.exit(1)
}

// Parse the fullTestRun array
const fullTestRunStr = fullTestRunMatch[1]
const fullTestRun = fullTestRunStr
  .split(',')
  .map((item) => item.trim())
  .filter((item) => item)
  .map((item) => item.replace(/['"]/g, ''))

console.log('Found fullTestRun with', fullTestRun.length, 'items')

// Build the detoxMapping object
const detoxMapping = {
  directory: {
    allergies: ['Allergies'],
...
// Write the JSON file
const outputPath = process.argv[2] || '/tmp/detoxMapping.json'
fs.writeFileSync(outputPath, JSON.stringify(detoxMapping, null, 2))
console.log(`Successfully created ${outputPath}`)
console.log(`Full test run contains ${fullTestRun.length} items`)
console.log(`Components array contains ${detoxMapping.directory.components.length} items`)
