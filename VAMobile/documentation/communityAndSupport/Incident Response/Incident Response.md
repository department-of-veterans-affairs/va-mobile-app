...
|**Minor/Low incident**|<ul><li>Impact is very localized or affects few Veterans</li><li>Some functionality is unavailable, but there’s a workaround</li><li>The site does not look the way it should, but it doesn’t affect functionality</li><li>Degradation in service to customers that are not Veterans</li></ul>|As a rule, no escalation is needed for minor incidents but the  Incident Commander can choose to escalate at their discretion.|
...
1. Determine the root cause 
    ...
    3. Don't forget about non-API dependencies such as the VA network gateways that sit in front of our backend services. If these are experiencing an issue it's almost certainly also affecting [VA.gov](va.gov) and possibly the VA as a whole.
