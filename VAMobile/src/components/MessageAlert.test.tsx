...
context('MessageAlert', () => {
  ...
  it('displays save draft success', async () => {
    initializeTestInstance(false, true, true, false, false, false)
    expect(testInstance.findAllByType(AlertBox).length).toEqual(1)
  })

  it('displays save draft validation', async () => {
    initializeTestInstance(true, true, false, false, false, false)
    expect(testInstance.findAllByType(AlertBox).length).toEqual(1)
  ...
  it('should show loading screen when saving draft', async () => {
    initializeTestInstance(false, true, false, false, true, false)
