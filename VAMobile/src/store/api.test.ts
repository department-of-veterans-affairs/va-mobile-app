...
context('api', () => {
	...
	it("should handle GET request params", async ()=> {
		fetch.mockResolvedValue({status:200, json:()=>Promise.resolve({foo:"test"})})
		let result = await call("GET", "/foo", {p1:"test", p2:"t&=$?est", ary:["123", "asdfasdf,d,asfd", "%%%"]})
		expect(result).toEqual(expect.objectContaining({foo:"test"}))
		// query params should be properly escaped
		expect(fetch).toHaveBeenCalledWith("/foo?p1=test&p2=t%26%3D%24%3Fest&ary=123&ary=asdfasdf%2Cd%2Casfd&ary=%25%25%25", expect.anything())
	})
	...
	it("should handle POST correctly", async ()=> {
		...
		expect(fetch).toHaveBeenCalledWith("/foo", expect.objectContaining({method:"POST", body, headers}))
