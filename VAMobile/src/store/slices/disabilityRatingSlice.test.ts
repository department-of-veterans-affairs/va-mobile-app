...
export const ActionTypes: {
  DISABILITY_RATING_START_GET_RATING: string
  DISABILITY_RATING_FINISH_GET_RATING: string
} = {
  DISABILITY_RATING_START_GET_RATING: 'disabilityRating/dispatchStartGetRating',
  DISABILITY_RATING_FINISH_GET_RATING: 'disabilityRating/dispatchFinishGetRating',
}
  ...
  describe('getDisabilityRating', () => {
    it('should get users disability ratings data', async () => {
      ...
      when(api.get as jest.Mock)
        .calledWith(url)
        .mockResolvedValue(mockDisabilityPayload)

      const store = realStore()
      await store.dispatch(getDisabilityRating())
      const actions = store.getActions()

      const startAction = _.find(actions, { type: ActionTypes.DISABILITY_RATING_START_GET_RATING })
      expect(startAction).toBeTruthy()
      expect(startAction?.state.disabilityRating.loading).toBeTruthy()

      const endAction = _.find(actions, { type: ActionTypes.DISABILITY_RATING_FINISH_GET_RATING })
