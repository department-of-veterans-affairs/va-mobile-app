// @ts-nocheck - this is provided and we don't want to have to redeclare it in several places
import {
  API_ROOT,
  APPLE_STORE_LINK,
  AUTH_SIS_ENDPOINT,
  AUTH_SIS_REVOKE_URL,
  AUTH_SIS_TOKEN_EXCHANGE_URL,
  AUTH_SIS_TOKEN_REFRESH_URL,
  CARRIER_TRACKING_URL_DHL,
  CARRIER_TRACKING_URL_FEDEX,
  CARRIER_TRACKING_URL_UPS,
  CARRIER_TRACKING_URL_USPS,
  DEMO_PASSWORD,
  ENVIRONMENT,
  GOOGLE_PLAY_LINK,
  IS_TEST,
  LINK_URL_ABOUT_DISABILITY_RATINGS,
  LINK_URL_ABOUT_PACT_ACT,
  LINK_URL_APPLY_FOR_HEALTH_CARE,
  LINK_URL_ASK_VA_GOV,
  LINK_URL_CLAIM_APPEAL_STATUS,
  LINK_URL_COMPENSATION_CLAIM_EXAM,
  LINK_URL_COPAY_HARDSHIP_INFO,
  LINK_URL_CONSUMER_GOV,
  LINK_URL_DECISION_REVIEWS,
  LINK_URL_DISPUTE_DEBT,
  LINK_URL_DISPUTE_COPAY,
  LINK_URL_GO_TO_BTSSS,
  LINK_URL_GO_TO_MY_HEALTHEVET,
  LINK_URL_GO_TO_PATIENT_PORTAL,
  LINK_URL_GO_TO_VA_GOV,
  LINK_URL_HELP_FROM_VA_ACCREDITED_REPRESENTATIVE,
  LINK_URL_HOW_TO_APPLY_FOR_HEALTH_CARE,
  LINK_URL_IN_APP_RECRUITMENT,
  LINK_URL_MHV_LABS_AND_TESTS,
  LINK_URL_MHV_VA_MEDICAL_RECORDS,
  LINK_URL_MHV_VA_MEDICATIONS,
  LINK_URL_MY_MONEY_GOV,
  LINK_URL_OMB_PAGE,
  LINK_URL_PRIVACY_POLICY,
  LINK_URL_REQUEST_HARDSHIP_ASSISTANCE,
  LINK_URL_REQUEST_HELP_FORM_5655,
  LINK_URL_REQUEST_HELP_VA_DEBT,
  LINK_URL_SCHEDULE_APPOINTMENTS,
  LINK_URL_TRAVEL_EXPENSE_DEDUCTIBLE,
  LINK_URL_TRAVEL_PAY_ELIGIBILITY,
  LINK_URL_TRAVEL_PAY_FILE_CLAIM_BTSSS,
  LINK_URL_TRAVEL_PAY_SET_UP_DIRECT_DEPOSIT,
  LINK_URL_TRAVEL_PAY_WEB_DETAILS,
  LINK_URL_UPGRADE_MY_HEALTHEVET_PREMIUM_ACCOUNT,
  LINK_URL_VA_FORM_10_3542,
  LINK_URL_VA_FORM_5655,
  LINK_URL_VA_MENTAL_HEALTH_SERVICES,
  LINK_URL_VA_NOTIFICATIONS,
  LINK_URL_VA_PRIVACY_POLICIES,
  LINK_URL_VA_SCHEDULING,
  LINK_URL_VETERANS_BENEFITS_BANKING,
  LINK_URL_VETERANS_CRISIS_LINE,
  LINK_URL_VETERANS_CRISIS_LINE_GET_HELP,
  LINK_URL_VETERAN_USABILITY_PROJECT,
  LINK_URL_WAIVERS_FOR_VA_BENEFIT_DEBT,
  LINK_URL_YOUR_CLAIMS,
  PAY_VA_GOV_URL,
  REACTOTRON_ENABLED,
  SHOW_DEBUG_MENU,
  SMHD_APPLE_STORE_LINK,
  SMHD_GOOGLE_PLAY_LINK,
  WEBVIEW_URL_APPOINTMENTS_CLAIM_EXAM_LEARN_MORE,
  WEBVIEW_URL_CHANGE_LEGAL_NAME,
  WEBVIEW_URL_CORONA_FAQ,
  WEBVIEW_URL_FACILITY_LOCATOR,
  WEBVIEW_URL_VIDEO_HEALTH_APPOINTMENTS,
  WEBVIEW_URL_WHAT_TO_BRING_TO_APPOINTMENTS,
} from '@env'

export type EnvVars = {
  API_ROOT: string
  APPLE_STORE_LINK: string
  AUTH_SIS_ENDPOINT: string
  AUTH_SIS_REVOKE_URL: string
  AUTH_SIS_TOKEN_EXCHANGE_URL: string
  AUTH_SIS_TOKEN_REFRESH_URL: string
  CARRIER_TRACKING_URL_DHL: string
  CARRIER_TRACKING_URL_FEDEX: string
  CARRIER_TRACKING_URL_UPS: string
  CARRIER_TRACKING_URL_USPS: string
  DEMO_PASSWORD: string
  ENVIRONMENT: string
  GOOGLE_PLAY_LINK: string
  IS_TEST: boolean
  LINK_URL_ABOUT_DISABILITY_RATINGS: string
  LINK_URL_ABOUT_PACT_ACT: string
  LINK_URL_APPLY_FOR_HEALTH_CARE: string
  LINK_URL_HELP_FROM_VA_ACCREDITED_REPRESENTATIVE: string
  LINK_URL_HOW_TO_APPLY_FOR_HEALTH_CARE: string
  LINK_URL_MHV_LABS_AND_TESTS: string
  LINK_URL_MHV_VA_MEDICAL_RECORDS: string
  LINK_URL_MHV_VA_MEDICATIONS: string
  LINK_URL_ASK_VA_GOV: string
  LINK_URL_CLAIM_APPEAL_STATUS: string
  LINK_URL_COPAY_HARDSHIP_INFO: string
  LINK_URL_COMPENSATION_CLAIM_EXAM: string
  LINK_URL_CONSUMER_GOV: string
  LINK_URL_DECISION_REVIEWS: string
  LINK_URL_DISPUTE_DEBT: string
  LINK_URL_DISPUTE_COPAY: string
  LINK_URL_GO_TO_BTSSS: string
  LINK_URL_GO_TO_MY_HEALTHEVET: string
  LINK_URL_GO_TO_PATIENT_PORTAL: string
  LINK_URL_GO_TO_VA_GOV: string
  LINK_URL_IN_APP_RECRUITMENT: string
  LINK_URL_MY_MONEY_GOV: string
  LINK_URL_OMB_PAGE: string
  LINK_URL_PRIVACY_POLICY: string
  LINK_URL_REQUEST_HARDSHIP_ASSISTANCE: string
  LINK_URL_SCHEDULE_APPOINTMENTS: string
  LINK_URL_UPGRADE_MY_HEALTHEVET_PREMIUM_ACCOUNT: string
  LINK_URL_VA_NOTIFICATIONS: string
  LINK_URL_VA_SCHEDULING: string
  LINK_URL_VETERAN_USABILITY_PROJECT: string
  LINK_URL_VETERANS_BENEFITS_BANKING: string
  LINK_URL_VETERANS_CRISIS_LINE_GET_HELP: string
  LINK_URL_VETERANS_CRISIS_LINE: string
  LINK_URL_YOUR_CLAIMS: string
  LINK_URL_TRAVEL_EXPENSE_DEDUCTIBLE: string
  LINK_URL_TRAVEL_PAY_ELIGIBILITY: string
  LINK_URL_TRAVEL_PAY_FILE_CLAIM_BTSSS: string
  LINK_URL_TRAVEL_PAY_SET_UP_DIRECT_DEPOSIT: string
  LINK_URL_TRAVEL_PAY_WEB_DETAILS: string
  LINK_URL_VA_FORM_10_3542: string
  LINK_URL_REQUEST_HELP_FORM_5655: string
  LINK_URL_REQUEST_HELP_VA_DEBT: string
  LINK_URL_VA_FORM_5655: string
  LINK_URL_VA_MENTAL_HEALTH_SERVICES: string
  LINK_URL_VA_PRIVACY_POLICIES: string
  LINK_URL_WAIVERS_FOR_VA_BENEFIT_DEBT: string
  REACTOTRON_ENABLED: boolean
  SHOW_DEBUG_MENU: boolean
  SMHD_APPLE_STORE_LINK: string
  SMHD_GOOGLE_PLAY_LINK: string
  WEBVIEW_URL_APPOINTMENTS_CLAIM_EXAM_LEARN_MORE: string
  WEBVIEW_URL_CORONA_FAQ: string
  WEBVIEW_URL_FACILITY_LOCATOR: string
  WEBVIEW_URL_CHANGE_LEGAL_NAME: string
  WEBVIEW_URL_WHAT_TO_BRING_TO_APPOINTMENTS: string
  WEBVIEW_URL_VIDEO_HEALTH_APPOINTMENTS: string
  PAY_VA_GOV_URL: string
}

// need to wrap @env for testing purposes
export default (): EnvVars => ({
  API_ROOT,
  APPLE_STORE_LINK,
  AUTH_SIS_ENDPOINT,
  AUTH_SIS_REVOKE_URL,
  AUTH_SIS_TOKEN_EXCHANGE_URL,
  AUTH_SIS_TOKEN_REFRESH_URL,
  CARRIER_TRACKING_URL_DHL,
  CARRIER_TRACKING_URL_FEDEX,
  CARRIER_TRACKING_URL_UPS,
  CARRIER_TRACKING_URL_USPS,
  DEMO_PASSWORD,
  ENVIRONMENT,
  GOOGLE_PLAY_LINK,
  IS_TEST: IS_TEST === 'true',
  LINK_URL_ABOUT_DISABILITY_RATINGS,
  LINK_URL_ABOUT_PACT_ACT,
  LINK_URL_APPLY_FOR_HEALTH_CARE,
  LINK_URL_HOW_TO_APPLY_FOR_HEALTH_CARE,
  LINK_URL_ASK_VA_GOV,
  LINK_URL_CLAIM_APPEAL_STATUS,
  LINK_URL_COPAY_HARDSHIP_INFO,
  LINK_URL_COMPENSATION_CLAIM_EXAM,
  LINK_URL_CONSUMER_GOV,
  LINK_URL_DECISION_REVIEWS,
  LINK_URL_DISPUTE_DEBT,
  LINK_URL_DISPUTE_COPAY,
  LINK_URL_GO_TO_BTSSS,
  LINK_URL_GO_TO_MY_HEALTHEVET,
  LINK_URL_GO_TO_PATIENT_PORTAL,
  LINK_URL_GO_TO_VA_GOV,
  LINK_URL_HELP_FROM_VA_ACCREDITED_REPRESENTATIVE,
  LINK_URL_IN_APP_RECRUITMENT,
  LINK_URL_MHV_LABS_AND_TESTS,
  LINK_URL_MHV_VA_MEDICAL_RECORDS,
  LINK_URL_MHV_VA_MEDICATIONS,
  LINK_URL_MY_MONEY_GOV,
  LINK_URL_OMB_PAGE,
  LINK_URL_PRIVACY_POLICY,
  LINK_URL_REQUEST_HARDSHIP_ASSISTANCE,
  LINK_URL_SCHEDULE_APPOINTMENTS,
  LINK_URL_UPGRADE_MY_HEALTHEVET_PREMIUM_ACCOUNT,
  LINK_URL_VA_NOTIFICATIONS,
  LINK_URL_VA_SCHEDULING,
  LINK_URL_VETERAN_USABILITY_PROJECT,
  LINK_URL_VETERANS_BENEFITS_BANKING,
  LINK_URL_VETERANS_CRISIS_LINE_GET_HELP,
  LINK_URL_VETERANS_CRISIS_LINE,
  LINK_URL_YOUR_CLAIMS,
  LINK_URL_TRAVEL_EXPENSE_DEDUCTIBLE,
  LINK_URL_TRAVEL_PAY_ELIGIBILITY,
  LINK_URL_TRAVEL_PAY_FILE_CLAIM_BTSSS,
  LINK_URL_TRAVEL_PAY_SET_UP_DIRECT_DEPOSIT,
  LINK_URL_TRAVEL_PAY_WEB_DETAILS,
  LINK_URL_VA_FORM_10_3542,
  LINK_URL_REQUEST_HELP_FORM_5655,
  LINK_URL_REQUEST_HELP_VA_DEBT,
  LINK_URL_VA_FORM_5655,
  LINK_URL_VA_MENTAL_HEALTH_SERVICES,
  LINK_URL_VA_PRIVACY_POLICIES,
  LINK_URL_WAIVERS_FOR_VA_BENEFIT_DEBT,
  REACTOTRON_ENABLED: REACTOTRON_ENABLED === 'true',
  SHOW_DEBUG_MENU: SHOW_DEBUG_MENU === 'true',
  SMHD_APPLE_STORE_LINK,
  SMHD_GOOGLE_PLAY_LINK,
  WEBVIEW_URL_APPOINTMENTS_CLAIM_EXAM_LEARN_MORE,
  WEBVIEW_URL_CORONA_FAQ,
  WEBVIEW_URL_FACILITY_LOCATOR,
  WEBVIEW_URL_CHANGE_LEGAL_NAME,
  WEBVIEW_URL_WHAT_TO_BRING_TO_APPOINTMENTS,
  WEBVIEW_URL_VIDEO_HEALTH_APPOINTMENTS,
  PAY_VA_GOV_URL,
})
