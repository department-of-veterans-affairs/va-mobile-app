...
const getAvsBinaries = async (
  ...
  return get<AvsBinariesGetData>(`/v0/appointments/avs_binaries/${appointmentId}`, {
    doc_ids: toFetch.map((item) => item.id).join(','),
  } as Params)
}
...
export const prefetchAvsBinaries = async (
  ...
  await queryClient.prefetchQuery({
    ...
    staleTime: ACTIVITY_STALE_TIME * 3, // 15 minutes - we want to avoid refetching these too often since they don't change and can be large files
  })
}
