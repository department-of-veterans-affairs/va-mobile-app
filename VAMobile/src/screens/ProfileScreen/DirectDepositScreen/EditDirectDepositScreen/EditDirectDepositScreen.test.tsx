  ...
  describe('when content is invalid', () => {
    it('should display an AlertBox and field errors', async () => {
      await waitFor(() => {
        ...
        expect(testInstance.findByType(TextView).props.children).toEqual("Enter the bank's 9-digit routing number.")
        expect(testInstance.findByType(TextView).props.children).toEqual('Enter your account number.')
        expect(testInstance.findByType(TextView).props.children).toEqual('Select the type that best describes the account.')
        expect(testInstance.findByType(TextView).props.children).toEqual('Confirm this information is correct.')
      })
    })
  })
