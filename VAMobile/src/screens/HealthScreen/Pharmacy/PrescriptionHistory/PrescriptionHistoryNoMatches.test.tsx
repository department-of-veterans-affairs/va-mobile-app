...
context('PrescriptionHistoryNoMatches', () => {
  it('should show tab based content with no filter', () => {
    ...
    expect(screen.getAllByText('We can’t find any VA prescriptions that match the criteria for this list.')).toBeTruthy()
  })

  it('should show tab based content for filtered lists', () => {
    render(<PrescriptionHistoryNoMatches isFiltered={true} />)
    expect(screen.getAllByText('We can’t find any VA prescriptions that match your filter selection. Try changing or resetting the filter.')).toBeTruthy()
  })
})
