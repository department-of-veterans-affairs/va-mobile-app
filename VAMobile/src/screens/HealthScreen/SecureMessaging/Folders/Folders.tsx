...
import { DELETED, HIDDEN_FOLDERS } from 'constants/secureMessaging'
  ...
  _.forEach(listOfFolders, (folder, index) => {
    ...
    if (!HIDDEN_FOLDERS.has(name)) {
      listItems.push({
        ...
        a11yHintText: t(folder.attributes.systemFolder ? `secureMessaging.${name.toLowerCase()}.a11yHint` : 'secureMessaging.folders.customFolders.a11yHint'),
...
export const getSystemFolders = (
  ...
  const systemFolders = _.filter(folders, (folder) => {
    return folder.attributes.systemFolder && folder.attributes.name !== DELETED
  })
