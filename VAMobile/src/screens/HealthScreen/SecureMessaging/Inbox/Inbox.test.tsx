...
context('Inbox', () => {
  ...
  const testInboxMessage = (category: CategoryTypes = CategoryTypeFields.other , subjectLine: string = 'Default subject line'): SecureMessagingMessageList => [{
    type: 'test',
    id: 1,
    attributes: {
      messageId: 1,
  ...
  const initializeTestInstance = (inboxMessages: SecureMessagingMessageList | undefined = undefined, loading: boolean = false) => {
    props = mockNavProps()
  ...
  describe('when there are inbox messages', () => {
    it('should not render NoInboxMessages', async () => {
      initializeTestInstance(testInboxMessage())
  ...
  describe('when loading is set to true', () => {
    it('should show loading screen', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.other, ''), true)
  ...
  describe('when subject line is empty', () => {
    it('should show only category with no colon or space after', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.other, ''))
  ...
  describe('when subject category is GENERAL', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.general))
      expect(testInstance.findAllByType(TextView)[2].props.children).toBe('General: Default subject line')
    })
  })
  ...
  describe('when subject category is APPOINTMENTS', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.appointment))
  ...
  describe('when subject category is MEDICATION', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.medication))
  ...
  describe('when subject category is TEST_RESULTS', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.test))
  ...
  describe('when subject category is EDUCATION', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.education))
  ...
  describe('when subject category is COVID', () => {
    it('should show correct text', async () => {
      initializeTestInstance(testInboxMessage(CategoryTypeFields.covid))
