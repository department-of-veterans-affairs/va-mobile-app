...
import { getDemographics, initialAuthorizedServicesState, initialAuthState, initialDisabilityRatingState, initialMilitaryServiceState, initialPersonalInformationState } from 'store/slices'
...
context('SyncScreen', () => {
  ...
  const initializeTestInstance = (militaryLoading = true, profileLoading = true, disabilityRatingLoading = true, loadingDemographics = true, loggedIn = false, loggingOut = false, syncing = true): void => {
    store = {
      ...
      personalInformation: { ...initialPersonalInformationState, preloadComplete: !profileLoading, demographicsPreloadComplete: !loadingDemographics },
      authorizedServices: {
  ...
  describe('sign out', () => {
    it('should show sign out text', async () => {
      initializeTestInstance(false, false, false, false, true, true)
    ...
    it('should show sign out text even if data is not loaded', async () => {
      initializeTestInstance(true, true, true, true, true, true)
      expect(testInstance.findByType(TextView).props.children).toEqual('Signing you out...')
    })
  })
  ...
  describe('loading sequence', () => {
    it('should load profile first if the user is logged in', async () => {
      initializeTestInstance(true, true, true, true, true, false)
    ...
    it('should load military history after profile has loaded', async () => {
      initializeTestInstance(true, false, true, true, true, false)
    ...
    it('should load disability ratings after military history has loaded', async () => {
      initializeTestInstance(false, false, true, true, true, false)
      expect(getProfileInfo).not.toHaveBeenCalled()
      expect(getServiceHistory).not.toHaveBeenCalled()
      expect(getDisabilityRating).toHaveBeenCalled()
    })

    it('should load demographics after disability ratings has loaded', async () => {
      initializeTestInstance(false, false, false, true, true, false)
      expect(getProfileInfo).not.toHaveBeenCalled()
      expect(getServiceHistory).not.toHaveBeenCalled()
      expect(getDisabilityRating).not.toHaveBeenCalled()
  ...
  describe('sync completion', () => {
    it('should complete the sync when all loading is finished', async () => {
      initializeTestInstance(false, false, false, false, true, false)
