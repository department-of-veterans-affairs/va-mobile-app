...
const content = fs.readFileSync('screenshot_data.ts', 'utf8');
...
for (const line of lines) {
    ...
    if (inObject) {
        ...
        if (descMatch) {
            ...
            if (desc.includes('[')) {
                currentDescription = desc.replace(/[[\]\',]/g, ' ').trim().replace(/\s+/g, '\n');
            } else {
    ...
    if (line.trim().startsWith('},')) {
        if (currentTestId) {
            for (const device in currentImages) {
                ...
                console.log(`${currentImages[device]}\t${currentTestId}\t${device}\t"${currentDescription}"`);
            }
        }
        inObject = false;
    }
